 ...
 * Versi Final - Dengan Referensi Otomatis Setiap Nyala
 * 
 * Fitur:
 ...
 * - REFERENSI OTOMATIS SETIAP ALAT DINYALAKAN
...
// ============================================
// VARIABEL REFERENSI (TAMBAHAN)
// ============================================
float refIR = 0;      // Nilai referensi IR (tanpa jari)
float refRed = 0;     // Nilai referensi Red (tanpa jari)
float refGreen = 0;   // Nilai referensi Hijau (tanpa jari)
...
// ============================================
// FUNGSI AMBIL REFERENSI OTOMATIS (TAMBAHAN)
// ============================================
void ambilReferensi() {
  lcd.clear();
  lcd.setCursor(0, 1);
  lcd.print("KALIBRASI REFERENSI");
  lcd.setCursor(0, 2);
  lcd.print("JANGAN ADA JARI!");
  lcd.setCursor(0, 3);
  lcd.print("Tunggu 3 detik...");
  
  delay(3000);  // Beri waktu user untuk tidak meletakkan jari
  
  // Ambil referensi IR (untuk Gula)
  lcd.setCursor(0, 3);
  lcd.print("Baca IR...        ");
  digitalWrite(ledIR, HIGH);
  delay(300);  // Tunggu LED stabil
  
  refIR = 0;
  for(int i=0; i<10; i++) {  // Baca 10 kali untuk rata-rata
    refIR += analogRead(sensorPin);
    delay(30);
  }
  refIR = refIR / 10.0;
  digitalWrite(ledIR, LOW);
  
  // Ambil referensi Merah (untuk Kolesterol)
  lcd.setCursor(0, 3);
  lcd.print("Baca Merah...     ");
  digitalWrite(ledRed, HIGH);
  delay(300);
  
  refRed = 0;
  for(int i=0; i<10; i++) {
    refRed += analogRead(sensorPin);
    delay(30);
  }
  refRed = refRed / 10.0;
  digitalWrite(ledRed, LOW);
  
  // Ambil referensi Hijau (untuk Asam Urat)
  lcd.setCursor(0, 3);
  lcd.print("Baca Hijau...     ");
  digitalWrite(ledGreen, HIGH);
  delay(300);
  
  refGreen = 0;
  for(int i=0; i<10; i++) {
    refGreen += analogRead(sensorPin);
    delay(30);
  }
  refGreen = refGreen / 10.0;
  ...
  lcd.print("REFERENSI:");
  lcd.setCursor(0, 1);
  lcd.print("IR: " + String(refIR, 0));
  lcd.setCursor(0, 2);
  lcd.print("Red: " + String(refRed, 0));
  lcd.setCursor(0, 3);
  lcd.print("Green: " + String(refGreen, 0));
  delay(1500);
  
  // Simpan ke EEPROM (alamat 100-112) untuk cadangan
  EEPROM.put(100, refIR);
  EEPROM.put(104, refRed);
  EEPROM.put(108, refGreen);
  
  Serial.println("Referensi tersimpan:");
  Serial.print("IR: "); Serial.println(refIR);
  Serial.print("Red: "); Serial.println(refRed);
  Serial.print("Green: "); Serial.println(refGreen);
}

// ============================================
// FUNGSI LOAD REFERENSI DARI EEPROM (UNTUK CADANGAN)
// ============================================
void loadReferensi() {
  EEPROM.get(100, refIR);
  EEPROM.get(104, refRed);
  EEPROM.get(108, refGreen);
  
  // Cek apakah nilai valid (antara 100-1024, tidak 0)
  if(refIR < 100 || refIR > 1024 || refRed < 100 || refRed > 1024 || refGreen < 100 || refGreen > 1024) {
    refIR = 0;
    refRed = 0;
    refGreen = 0;
    Serial.println("Referensi EEPROM tidak valid");
  } else {
    Serial.println("Referensi dimuat dari EEPROM:");
    Serial.print("IR: "); Serial.println(refIR);
    Serial.print("Red: "); Serial.println(refRed);
    Serial.print("Green: "); Serial.println(refGreen);
  }
...
void setup() {
  ...
  // ============================================
  // AMBIL REFERENSI OTOMATIS SETIAP NYALA
  // ============================================
  ambilReferensi();
...
// ============================================
// FUNGSI BACA SENSOR DENGAN REFERENSI
// ============================================
...
void readAllSensors() {
  ...
  // ============================================
  // BACA SENSOR DENGAN REFERENSI YANG SUDAH ADA
  // ============================================
  ...
  // ============================================
  // HITUNG ABSORBANSI DENGAN REFERENSI
  // Rumus: Absorbansi = log(I0/I)
  // I0 = nilai referensi (tanpa jari)
  // I  = nilai dengan jari
  // ============================================
  
  // Cegah division by zero atau log negatif
  ...
  // Konversi ke mg/dL (rumus dasar)
  float rawGlucose = 42.3 + (352.0 * absorbIR);
  float rawCholesterol = 98.5 + (645.0 * absorbRed);
  float rawUricAcid = 2.1 + (52.3 * absorbGreen);
  ...
  Serial.print(" (ref:"); Serial.print(refIR);
  Serial.print(") absorb:"); Serial.println(absorbIR, 4);
  
  Serial.print("Red: "); Serial.print(redValue);
  Serial.print(" (ref:"); Serial.print(refRed);
  Serial.print(") absorb:"); Serial.println(absorbRed, 4);
  
  Serial.print("Green: "); Serial.print(greenValue);
  Serial.print(" (ref:"); Serial.print(refGreen);
  Serial.print(") absorb:"); Serial.println(absorbGreen, 4);
}
...
float readKalibrasiSensor(int param) {
  ...
  switch(param) {
    case 0: // Gula
      ...
      if(value <= 0) value = 1;
      return log(refIR / value);  // Kembalikan absorbansi
    ...
    case 2: // Asam Urat
      ...
      if(value <= 0) value = 1;
      return log(refGreen / value);
      
    case 3: // Suhu
      sensors.requestTemperatures();
      value = sensors.getTempCByIndex(0);
      if(value < 20 || value > 45 || value == -127) {
        value = 36.5;
      }
      return value;  // Suhu tidak perlu absorbansi
  }
...
void hitungKoefisienKalibrasi(int param) {
  ...
  // y = nilai konvensional, x = nilai alat (absorbansi)
